"""
LSUN (church, bedroom, cat) 256x256 training and sampling script for DDPM.

Migrated from: diffusion_tf/scripts/run_lsun.py
Migration notes:
- TF training loop -> PyTorch training loop
- TPU -> DDP multi-GPU training  
- NHWC -> NCHW data format
- TFRecords -> LMDB/folder dataset

Usage:
    # Training LSUN Bedroom
    python scripts/run_lsun.py train \
        --log_dir ./logs/lsun_bedroom \
    ...
    # Training LSUN Church
    python scripts/run_lsun.py train \
        --log_dir ./logs/lsun_church \
        --data_dir ./data/lsun/church_outdoor \
        --category church_outdoor \
        --batch_size 32

    # Training LSUN Cat (no random flip for cats)
    python scripts/run_lsun.py train \
        --log_dir ./logs/lsun_cat \
        --data_dir ./data/lsun/cat \
        --category cat \
        --randflip 0

    # Multi-GPU Training
    torchrun --nproc_per_node=4 scripts/run_lsun.py train \
        --log_dir ./logs/lsun_bedroom_ddp \
        --data_dir ./data/lsun/bedroom \
        --batch_size 64

    # Sampling
    python scripts/run_lsun.py sample \
        --checkpoint ./logs/lsun_bedroom/checkpoints/ema_500000.pt \
...
class LSUNDataset(Dataset):
    ...
    1. Folder of images (*.png, *.jpg, *.webp)
    2. LMDB format (standard LSUN format)
    """
    ...
    def __init__(self, data_dir: str, image_size: int = 256, randflip: bool = True):
        ...
        if os.path.exists(lmdb_path) or os.path.exists(os.path.join(data_dir, 'lock.mdb')):
            self._init_lmdb()
        else:
            self._init_folder()
    
    def _init_folder(self):
        """Initialize from folder of images."""
        import glob
        self.image_files = []
        for ext in ['*.png', '*.jpg', '*.jpeg', '*.webp', '*.PNG', '*.JPG', '*.WEBP']:
            self.image_files.extend(glob.glob(os.path.join(self.data_dir, '**', ext), recursive=True))
    ...
    def _init_lmdb(self):
        ...
        try:
            ...
            self.keys = None  # Lazy load keys
            print(f"Loaded LMDB with {self.length} images from {self.data_dir}")
        except ImportError:
            print("lmdb not installed, falling back to folder mode")
            self._init_folder()
    
    def _load_keys(self):
        """Lazy load all keys from LMDB."""
...
def get_default_config():
    """
    Get default configuration for LSUN training.
    
    Matches TF config: ch=128, ch_mult=(1, 1, 2, 2, 4, 4), attn_resolutions=(16,)
    Same as CelebA-HQ: unet2d16b2c112244 with 114M params
    """
    return {
        # Model
        'ch': 128,
        ...
        'num_channels': 3,
        'randflip': True,
        'category': 'bedroom',  # bedroom, church_outdoor, cat
...
def space_to_depth(x, block_size):
    """[B, C, H, W] -> [B, C*block^2, H/block, W/block]"""
    if block_size == 1:
        return x
    B, C, H, W = x.shape
...
def depth_to_space(x, block_size):
    """[B, C*block^2, H, W] -> [B, C, H*block, W*block]"""
    if block_size == 1:
        return x
    B, C, H, W = x.shape
...
def train(
    ...
    if is_main_process():
        ...
        print(f"Training on LSUN {config['category']}")
...
def parse_args():
    parser = argparse.ArgumentParser(description='DDPM LSUN Training/Sampling')
    ...
    train_parser.add_argument('--log_dir', type=str, default='./logs/lsun')
    train_parser.add_argument('--data_dir', type=str, default='./data/lsun/bedroom')
    train_parser.add_argument('--category', type=str, default='bedroom',
                              choices=['bedroom', 'church_outdoor', 'cat'],
                              help='LSUN category')
    ...
    train_parser.add_argument('--block_size', type=int, default=1)
    train_parser.add_argument('--randflip', type=int, default=1, help='Random horizontal flip (0 or 1)')
    ...
    sample_parser.add_argument('--output_dir', type=str, default='./samples/lsun')
...
def main():
    ...
    if args.mode == 'train':
        ...
        config['randflip'] = bool(args.randflip)
        ...
        print("Usage:")
        print("  # LSUN Bedroom")
        print("  python run_lsun.py train --log_dir ./logs/lsun_bedroom --data_dir ./data/lsun/bedroom")
        print("")
        print("  # LSUN Church")
        print("  python run_lsun.py train --log_dir ./logs/lsun_church --data_dir ./data/lsun/church_outdoor")
        print("")
        print("  # LSUN Cat (no flip)")
        print("  python run_lsun.py train --log_dir ./logs/lsun_cat --data_dir ./data/lsun/cat --randflip 0")
        print("")
        print("  # Sampling")
        print("  python run_lsun.py sample --checkpoint ./logs/lsun_bedroom/checkpoints/ema_500000.pt")
